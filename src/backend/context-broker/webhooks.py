...
class WebhookBase:
    ...
    def send_to_influxdb(self, data):
        try:
            ...
            elif self.topic == "station/patras":
                point = Point(self.measurement) \
                    .tag("id", str(payload['id'])) \
...
class CarWebhook(WebhookBase):
    def __init__(self):
        super().__init__(topic="car", measurement="car_metrics", logger_name="CAR-WEBHOOK")

class StationWebhook(WebhookBase):
    def __init__(self):
        super().__init__(topic="station", measurement="station_aqi", logger_name="STATION-WEBHOOK")

class PatrasStationWebhook(WebhookBase):
    def __init__(self):
        super().__init__(topic="station/patras", measurement="patras_station_aqi", logger_name="STATION-PATRAS-WEBHOOK")
